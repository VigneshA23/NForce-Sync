...
export default function EmployeeEodInbox() {
  ...
  const filteredRows = useMemo(() => sortEodInboxRows(
    rows
      ...
      .filter(r => eodInboxMatchesFilters(r, { project: projectFilter, category: categoryFilter })),
    sort,
  ), [rows, filter, search, projectFilter, categoryFilter, sort]);
        ...
        {isPending ? (
          ...
          <Card style={{ padding: '48px 20px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--txt)', marginBottom: 4 }}>
              {search.trim() || projectFilter.size || categoryFilter.size ? 'No threads match your filters' : 'Nothing here yet'}
            </div>
            <div style={{ fontSize: 12.5, color: 'var(--txt-dim)' }}>
              {search.trim() || projectFilter.size || categoryFilter.size ? 'Try a different name, project or keyword.' : EMPTY_MESSAGE[filter]}
            </div>
          </Card>
        ) : (
          <EodInboxTable
