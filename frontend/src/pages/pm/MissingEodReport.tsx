...
// ── gaps calendar (grouped by month, click-to-select MISSED days) ──────────────
...
function leadingEmptyCount(firstIso: string): number {
  const dow = new Date(firstIso + 'T12:00:00').getDay(); // 0 = Sun
    ...
    <div>
      ...
      {isError ? (
        ...
        <Card style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr 1fr 1fr 90px 160px', padding: '10px 16px', fontSize: 10, fontWeight: 700, letterSpacing: '0.06em', color: 'var(--txt-dim)', textTransform: 'uppercase', borderBottom: '1px solid var(--line)', background: 'var(--raised)' }}>
            <div>Employee</div><div>Project</div><div>Team Lead</div><div>Missing</div><div>Status</div><div />
          </div>
          {isLoading ? (
            ...
            pageRows.map(row => (
              <div key={row.employeeId} style={{ display: 'grid', gridTemplateColumns: '1.6fr 1fr 1fr 1fr 90px 160px', padding: '10px 16px', alignItems: 'center', borderBottom: '1px solid var(--line)', fontSize: 12.5 }}>
                ...
                <div style={{ color: 'var(--txt-mut)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.teamName ?? '—'}</div>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          <AlertTriangle size={16} style={{ color: 'var(--warn)', marginTop: 1, flexShrink: 0 }} aria-hidden="true" />
          <p style={{ margin: 0, fontSize: 13, color: 'var(--txt)' }}>
            Send a reminder to all {employees.length} employees currently listed, each about their own missing days.
          </p>
        </div>
      </Modal>
